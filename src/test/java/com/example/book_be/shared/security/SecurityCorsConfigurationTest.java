...
class SecurityCorsConfigurationTest {
    ...
    @Test
    void exposes_auth_csrf_header_and_preserves_credentials_policy() {
        SecurityConfiguration securityConfiguration = new SecurityConfiguration();
        CorsConfiguration configuration = securityConfiguration.corsConfigurationSource(
                        new FrontendUrlProvider("https://frontend.example"))
                .getCorsConfiguration(new MockHttpServletRequest());
        ...
        assertThat(configuration.getExposedHeaders()).containsExactly(RequestTraceFilter.HEADER_NAME);
        assertThat(configuration.getAllowCredentials()).isTrue();
        assertThat(configuration.getMaxAge()).isEqualTo(3600L);
    }

    @Test
    void session_endpoints_are_exact_and_no_broad_tai_khoan_matcher_exists() {
        assertThat(Endpoints.PUBLIC_GET_ENDPOINS).doesNotContain("/tai-khoan/**");
        assertThat(Endpoints.PUBLIC_POST_ENDPOINS).doesNotContain("/tai-khoan/**");
        assertThat(Endpoints.AUTH_GET_ENDPOINTS).doesNotContain("/tai-khoan/**");
        assertThat(Endpoints.AUTH_POST_ENDPOINTS).doesNotContain("/tai-khoan/**");
    }

    @Test
    void vnpay_callback_is_matched_as_public_before_protected_order_routes() {
        assertThat(Endpoints.PUBLIC_GET_ENDPOINS)
                .contains("/api/don-hang/vnpay-payment");
        assertThat(Endpoints.AUTH_GET_ENDPOINTS)
                .contains("/api/don-hang/**");
    }
}
